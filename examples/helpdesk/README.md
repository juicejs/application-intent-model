...
A small support-ticket app that exercises **every** AIM v5.5 feature in one connected graph. It is intentionally split across files, because the point of the graph model is that the relation graph *spans files* — and you can still traverse, check, and diff it.
