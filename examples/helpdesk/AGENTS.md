...
Roles: the **Architect** authors intent, the **Developer** realizes it in code, the **Reviewer** diffs reality against the declared graph. Realization detail (symbols, routes, tables) belongs in `kind: binding` files only — behavioral files never carry code paths.
