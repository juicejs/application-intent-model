...
Use these prompts to initialize an AI agent (Claude Code, Cursor, Aider, etc.) into a specific AIM v4 role. Each prompt grounds the agent in the v4 specification (`specification.md`, also published at <https://intentmodel.dev/spec.md>) and reads local context before performing any action.
...
```md
Initialize as an AIM v4 Architect Agent by reading your instructions in `brain/brain.architect.md` and the language specification in `specification.md` (or https://intentmodel.dev/spec.md).

Read any existing `.aim` files under `./aim/` for [Component Name]. Once ready, confirm you have parsed the v4 specification and summarized the existing intent context (or noted its absence). Do not draft any new intent yet — wait for me to describe the requirements.
```
...
Read the intent files in `./aim/[Component Name]/`, build the declared graph, and scan the current implementation in the codebase. Once ready, confirm you have parsed the v4 specification and mapped the relationship between the declared graph and the implementation. Do not produce a drift report yet.
```
