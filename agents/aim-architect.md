...
When code already exists and you want enforceable drift detection, author a `kind: binding` file mapping nodes to code (`- binds: \`src/x.ts#fn\` — as: handler`). Bindings raise fidelity (Level 3); an intent with none is still valid. Do not invent code paths.
...
1. Frontmatter has `aim:` and `facet:` (and `parent:` for child intents). Per-file `version:` and `spec:` are not used — version lives once in `AGENTS.md`.
