...
**Writes:** `.aim` files only — parent intent files, sub-intent intent files, facet files, and (when realization is known) binding files.

**Rules:**
- Express requirements explicitly in intent rather than leaving them implicit.
- Default to splitting: create the parent intent with shared concerns, then a sub-intent per feature.
...
- **Reuse, don't regenerate.** Before defining a `Schema`, `Persona`, or other entity, search the existing graph for one of that kind and name. If it exists, reference it (Imports + edge) instead of redefining — cross-cutting entities belong in one canonical home (e.g. `<app>.core`). Duplicate `User`s across files are how the model rots at scale (§15.8).
- **Keep the parent lean; extract shared facets.** A parent intent file is an index (Summary, Requirements, `## Subintents`, Dependencies) — not a container. Author shared schemas/personas/views as their own files (sibling facet files or `<app>.core`), never embedded en masse in one file. Don't dodge duplication by building a monolith — and make sure a canonical entity sits where its consumers can resolve it (an ancestor or `<app>.core`, not a sibling).
- **Evolve by transform, not rewrite.** Every change is an EXTEND of an existing intent or an ADD of a new one (§16). When an EXTEND grows an intent past one clear behavior (§4.3), **promote** the new capability into its own sub-intent; re-home a misplaced node, merge a true duplicate, split a two-behavior intent, rename for clarity. Each transform re-points every inbound edge, updates the parent `## Subintents` index, re-establishes path/header identity (§4.4), and relocates bindings with the code locator unchanged (§16.3) — so reshaping intent is a traceable graph-diff (§16.4), not an opaque rewrite.
- **UI pieces have fluid granularity.** A tab/panel/widget is `### Display` prose in its host view when simple, and is **promoted** into its own sub-intent once it earns a contract/schema/action (§15.9). There is no composition (`embeds`) verb — a host connects to a promoted piece via `reads`/`exposes`/`invokes`/`navigates`; inline layout is realization, not an intent edge.
...
4. Decide the decomposition: group the graph into intents — is this one feature or several? List candidate sub-intents. (Decomposition partitions the graph; it does not replace it.)
5. Write the **parent intent file** first: cross-cutting requirements, shared schemas, the `## Subintents` index.
6. For each feature, create a **sub-intent intent file** with its own requirements, tests, and facets.
...
**Refining an existing model:** every change is an EXTEND or an ADD (§16). If an EXTEND crosses the §4.3 "one clear behavior" line, **promote** the new capability into its own sub-intent rather than piling facets on the parent. Apply the transform (promote / split / re-home / merge / rename) so the result stays well-formed: re-point inbound edges, update the parent's `## Subintents`, fix path/header identity, and move any bindings (code locator unchanged). The output is a structured graph-diff, not a rewrite.
...
- `## Summary` / `## Requirements` / `## Tests` / `## Subintents` / `## Dependencies`
...
When code already exists and you want enforceable drift detection, author a `kind: binding` file mapping nodes to code (`- binds: \`src/x.ts#fn\` — as: handler`). Bindings raise fidelity (Level 3); a intent with none is still valid. Do not invent code paths.
...
1. Frontmatter has `aim:` and `facet:` (and `parent:` for sub-intents). Per-file `version:` and `spec:` are not used — version lives once in `AGENTS.md`.
...
5. Sub-intent files declare `parent:` matching an existing parent intent file.
