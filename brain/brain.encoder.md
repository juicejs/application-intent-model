...
Before writing any file, read the v5 specification.
...
Sections you will lean on hardest: §2 (graph model and the two projections), §4–§5 (layout, sub-intents), §7–§8 (facets, typed edges), §10 (bindings), §12 (diagnostics), §15 (practical guidance), §16 (transforms), and above all **§17 (re-encoding)** — it defines this role's direction, sources, provenance, and authority rules.
...
│   │                          #   each a lean index: Summary, Requirements, ## Subintents
│   ├── <capability>           # sub-intents: one clear behavior each (§4.3)
│   └── <capability>
└── <domain>
```
...
- **Table-of-contents test:** each intent's children must read as a short table of contents for the level below ("My Library has Playlists"). 3–9 children per level; three levels is the practical maximum (§5.5).
- **Noun-cluster rule (§12.2):** a Schema plus several like-named Contracts (often a View too) is a sub-intent — never leave the cluster lying flat inside a mixed intent.
...
**Phase A — SURVEY (read only).** Inventory the system's observable surfaces, grouped by capability: screens and routes a human reaches; API endpoints; stored entities (models, migrations); background jobs, schedules, webhooks; events and queues; outbound artifacts (mail, exports, files). Note who acts where — roles, guards, authz checks — these become Personas and `### Authz`.

**Phase B — DESIGN THE TREE, THEN STOP.** Propose the complete skeleton: the root purpose in one sentence, each domain and sub-intent with one line on what it will hold, every §2 shape rule applied. **Present the tree and wait for approval before encoding anything.** The tree is the highest-leverage decision of the whole pass and the cheapest to change at this moment; encoding against an unapproved tree wastes everything downstream.
